...
void Graph::handleButton() {
  ...
  if (buttonText.contains("AC")) {
    ...
    /*QString str1 = mainLineEdit->text();
    QByteArray ba = str1.toLocal8Bit();
    char *c_str2 = ba.data();*/
    int flag = 0;
    double resultDouble = 0;

    calc(("(" + mainLineEdit->text() + ")").toLocal8Bit().data(),
             xFromLineEdit->text().toLocal8Bit().data(), &flag);
    if (flag != 0) xLineEdit->setText(QString("INCORRECT_STRING:%1").arg(flag));
    // else mainLineEdit->setText(QString("%1").arg(resultDouble));
    is_result = 1;
    ...
    for (int i = 0; i < 1000; i++) {
      resultDouble = (calc(
          ("(" + mainLineEdit->text() + ")").toLocal8Bit().data(),
          QString::number(doubleFrom + step * i).toLocal8Bit().data(), &flag));
