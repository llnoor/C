...
int main (int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        for (int i = 1; i < argc; i++) {
            if ( argv[i][0]!='-') {
                ...
                if (file!=NULL) {
                    ...
                    length_arr = get_line_count(length_arr, &max_length_arr, file, &line_count);
                    if (length_arr!=NULL) {
                        string_arr = realloc(string_arr, line_count * sizeof(char *));
                        
                        string_arr = input_text(string_arr, prev_line_count, file, &line_count, length_arr);
                        //printf("line_count:%d\n", line_count);
                    } else {
                        printf("cat: %s: Empty file\n", argv[i]);
                        no_error = 0;
                    }
                    fclose(file);
                } else {
                    length_arr[line_count] = 36;
                    string_arr[line_count] = malloc(length_arr[line_count]*sizeof(char));
                    char *temp = "s21_cat: No such file or directory\n";
                    for (int n = 0; n < length_arr[line_count]; n++){
                        string_arr[line_count][n]=temp[n];
                    }
                    line_count++;
        ...
        //if (length_arr[line_count-1]==0) line_count--;
...
int get_flags(int argc, char **argv, struct flag_struct* cat_flag) {
    ...
    while ( (rez = getopt_long(argc, argv, short_options, long_options, &optindex)) != -1 ) {
        switch(rez) {
            case 'v': cat_flag->v_flag = 1; break;
            case 'b': cat_flag->b_flag = 1; cat_flag->n_flag = 0; break;
            case 'e': cat_flag->e_flag = 1; cat_flag->v_flag = 1; break;
            case 'E': cat_flag->e_flag = 1; break;
            case 'n': if (cat_flag->b_flag == 0) cat_flag->n_flag = 1; break;
            case 's': cat_flag->s_flag = 1; break;
            case 't': cat_flag->t_flag = 1; cat_flag->v_flag = 1; break;
            case 'T': cat_flag->t_flag = 1; break;
        }
    }
    return 0;
}

int cat_text(char **string_arr, int *line_count, int* length_arr, struct flag_struct* cat_flag) {
...
void n_text(char **string_arr, int line_count, int* length_arr) {
    for (int it = 0; it < line_count; it++) {
        ...
        int add_i = 7;
        char temp[12]; // it is int => argument in the range [1, 2147483647] = > 12 bytes
        ...
        string_arr[it] = realloc (string_arr[it], length_arr[it] * sizeof *string_arr[it]);
        for (int i=0; i < length_arr[it]; i++){
            string_arr[it][i]=int_str[i];
        }
        free(int_str);
...
void v_text(char **string_arr, int line_count, int* length_arr) {
    for (int it = 0; it < line_count; it++) {
        ...
        if ( number_of_t > 0 ) {
            ...
            int add_i=0;
            for (int i=0; i<length_arr[it]-1; i++) {
                /*if ( string_arr[it][i] == (int) 0 ) {
                    int_str[i+add_i] = '^';
                    int_str[i+(++add_i)] = '@';
                } else */
                if ( string_arr[it][i] < 32 && string_arr[it][i] !=9 /*&& string_arr[it][i] !=0*/ ) {
                    int_str[i+add_i] = '^';
