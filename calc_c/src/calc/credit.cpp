...
void Credit::handleButton() {
  ...
  if (loanAmountQLineEdit->text().toDouble() <= 0 ||
    ...
    if (typeQComboBox->currentIndex() == 0) {
      ...
      annuity(loanAmountQLineEdit->text().toDouble(),
                  periodQLineEdit->text().toInt(),
                  interestRateQLineEdit->text().toDouble(), &monthly_payment,
                  &over_payment, &total_payment, string);
      monthlyPaymentQLineEdit->setText(QString("%1").arg(monthly_payment));
    } else {
      double monthly_payment_first = 0.0;
      double monthly_payment_last = 0.0;
      differentiated(loanAmountQLineEdit->text().toDouble(),
                         periodQLineEdit->text().toInt(),
                         interestRateQLineEdit->text().toDouble(),
                         &monthly_payment_first, &monthly_payment_last,
                         &over_payment, &total_payment, string);
      monthlyPaymentQLineEdit->setText(QString("%1...%2")
                                           .arg(monthly_payment_first)
                                           .arg(monthly_payment_last));
    }

    overPaymentQLineEdit->setText(QString::number(over_payment, 'f', 2));
    totalPaymentQLineEdit->setText(QString::number(total_payment, 'f', 2));
    editQTextEdit->setPlainText(QString::fromLocal8Bit(string));
  }
  free(string);
}
